...
class ICIMV13WorkflowTests(unittest.TestCase):
    def test_workflow_uses_r6_migration_persistence_and_gmail_gate(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("name: IC IM v1.3-r6 Post-Close Digest", text)
        self.assertIn('- cron: "0 10 * * *"', text)
        self.assertNotIn('- cron: "3 5 * * *"', text)
        self.assertNotIn('- cron: "18 5 * * *"', text)
        self.assertNotIn('- cron: "33 5 * * *"', text)
        self.assertIn('group: ic-im-v1-3-r6-post-close-digest', text)
        ...
        self.assertIn("migrate_ic_im_v1_3_r5_to_r6_state.py", text)
        self.assertIn("--artifact-name ic-im-v1-3-ledger", text)
        ...
        self.assertIn("name: ic-im-v1-3-r6-ledger", text)
