...
import mail_utils
import check_ic_im_v1_4_delivery as ic_gate
import check_microcap_delivery as micro_gate
import prepare_ic_im_v1_4_marker as marker
import restore_ic_im_v1_4_ledger as restore
import restore_ic_im_v1_3_ledger as legacy_restore
    ...
    def get(req, **kwargs):
        ...
        if '/artifacts?' in url:
            ...
            payload = {'id':run, 'status':'completed', 'conclusion':'failure' if run == 3 else 'success',
                       'head_branch':'feature' if run == 2 else 'main', 'path':'.github/workflows/ic-im-v1-4-daily-digest.yml'}
        response = MagicMock()
...
def test_legacy_marker_requires_exact_metadata_mode_and_date(monkeypatch, mode):
    ...
    monkeypatch.setattr(legacy_restore, 'download', lambda *a: data.getvalue())
