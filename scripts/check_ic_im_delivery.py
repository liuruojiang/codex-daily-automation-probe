...
def fetch_artifacts(repository: str, token: str, name: str, api_url: str) -> dict[str, object]:
    quoted = urllib.parse.quote(name, safe="")
    url = f"{api_url.rstrip('/')}/repos/{repository}/actions/artifacts?name={quoted}&per_page=100"
...
def main() -> int:
    ...
    name = marker_name(day)
    exists = False
    if not args.correction:
        if not args.repository or not args.token:
            raise SystemExit("GITHUB_REPOSITORY and GITHUB_TOKEN are required")
        exists = marker_exists(
            fetch_artifacts(args.repository, args.token, name, args.api_url), name
        )
    write_outputs(
        {
